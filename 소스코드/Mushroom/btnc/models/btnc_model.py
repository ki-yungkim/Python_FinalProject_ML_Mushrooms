    ...
    def infoRequest(self, q1):
        ...
        if code == '00':
            ...
            for item in items:
                ...
                # 일본명
                jpnname = item.find('jpnname').text
                # 북한명
                prkname = item.find('prkname').text
                # 학명ID
                fngsScnmId = item.find('fngsScnmId').text
                # 저작권설명
                cprtCtnt = item.find('cprtCtnt').text

                results.append([fngsAthrNm, fngsGenusNm, fngsTtnm, fngsScnm, engname, korname, jpnname, prkname, fngsScnmId, cprtCtnt])
