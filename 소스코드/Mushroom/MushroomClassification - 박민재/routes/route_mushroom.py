...
def test03input_B():
    print("Call test03search_image")
    listdir = os.listdir("static/uploads/test/")

    for i in range(len(listdir)):
        print(listdir[i])
        if listdir[i] != ".DS_Store":
            os.remove("static/uploads/test/"+listdir[i])
        else:
            continue
...
def test03resultImage():
    ...
    inputImage.save("static/uploads/test/"+(inputImage.filename))
